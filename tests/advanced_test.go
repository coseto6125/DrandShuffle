...
func TestStringToCardErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := drandshuffle.StringToCard(tc.cardString)
			assert.Error(t, err)
...
func TestShuffleDeckWithInsufficientRandomness(t *testing.T) {
	t.Run("Test shuffle with insufficient randomness", func(t *testing.T) {
		...
		// 使用不足的隨機數據洗牌
		shuffled := drandshuffle.ShuffleDeck(deck, randomness)
		...
		for _, card := range shuffled {
			shuffledCards[drandshuffle.CardToString(card)] = true
		}
...
func TestShuffleReproducibility(t *testing.T) {
	t.Run("Test shuffle reproducibility with same randomness", func(t *testing.T) {
		...
		// 使用相同的隨機數據洗牌兩次
		shuffled1 := drandshuffle.ShuffleDeck(deck, finalRandomness)
		shuffled2 := drandshuffle.ShuffleDeck(deck, finalRandomness)

		// 檢查兩次洗牌結果是否相同
		for i := 0; i < len(shuffled1); i++ {
			assert.Equal(t,
				drandshuffle.CardToString(shuffled1[i]),
				drandshuffle.CardToString(shuffled2[i]),
	...
	t.Run("Test shuffle uniqueness with different game session IDs", func(t *testing.T) {
		...
		// 使用不同的遊戲局號洗牌
		shuffled1 := drandshuffle.ShuffleDeck(deck, finalRandomness1)
		shuffled2 := drandshuffle.ShuffleDeck(deck, finalRandomness2)
